...
class AudioLoader:
    ...
    def load(self, file_path: str | Path) -> Tuple[AudioInput, np.ndarray]:
        ...
        try:
            # Convert WebM/OGG to WAV if needed (using pydub with ffmpeg)
            if file_ext in [".webm", ".ogg"]:
                ...
                if magic_bytes[:4] == b'\x1a\x45\xdf\xa3':
                    format_name = "webm"
                    logger.info(f"Detected WebM format (EBML header), converting to WAV...")
                elif magic_bytes[:4] == b'OggS':
                    format_name = "ogg"
                    logger.info(f"Detected OGG format, converting to WAV...")
                else:
                    # Fallback to extension-based detection
                    format_name = "webm" if file_ext == ".webm" else "ogg"
                    logger.warning(f"Unknown magic bytes {magic_bytes.hex()}, assuming {format_name.upper()}")
